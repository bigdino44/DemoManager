...
export function AnalyticsView() {
  ...
  const metrics = [
    ...
    {
      title: 'Avg. Attendees',
      value: Math.round(demos.reduce((acc, demo) => acc + demo.attendees, 0) / demos.length || 0),
      change: '+8%',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm">
          ...
          <div className="space-y-6">
            {Object.entries(demoTypes).map(([key, type], index) => {
              const demoCount = demos.filter(demo => demo.location.toLowerCase() === key).length;
              const percentage = (demoCount / demos.length) * 100 || 0;
              ...
              return (
                <div key={key} className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    {type.cost && (
                      <div className="text-sm text-gray-600">Cost: {type.cost}</div>
                    )}
                  </div>
